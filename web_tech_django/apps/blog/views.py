...
def about(request):
    if request.method == 'POST':
        ...
        subject = 'Welcome to Kigali Tours'
...
def tours(request):
    ...
    # tours.count()
    return render(request, 'blog/tours.html', context)

def tours_list_all(request):
    tours = Tour.objects.all().values()  # Fetch all tours as dictionaries
...
def hotels_list(request):
    hotels = Hotel.objects.all()
    paginator = Paginator(hotels, 10)  # Show 10 tours per page
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    context = {'page_obj': page_obj}
    return render(request, 'admin/hotel/hotels_list_admin.html', context)
...
def hotels_list_all(request):
    hotels = Hotel.objects.all().values()  # Fetch all tours as dictionaries
